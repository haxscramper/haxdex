                    ...
                    for stmt in child.body:
                        ...
                        for tgt in targets:
                            if isinstance(tgt, ast.Name):
                                ...
                                if key in mapping and mapping[key] != enum_class:
                                    collisions.add(key)
...
def main():
    ...
    parser.add_argument("references", nargs="+", help="One or more .pyi stub files")
    parser.add_argument("-t", "--target", required=True, help="Python file to rewrite")
    ...
    for ref in args.references:
        with open(ref, "r", encoding="utf-8") as f:
            collect_mapping(ast.parse(f.read(), filename=ref), mapping, collisions)

    for key in sorted(collisions):
        print("warning: ambiguous member {}.{}, skipping".format(*key), file=sys.stderr)
    ...
    with open(args.target, "w", encoding="utf-8") as f:
        f.write(result)
    print("rewrote {} ({} replacements)".format(args.target, len(edits)), file=sys.stderr)
